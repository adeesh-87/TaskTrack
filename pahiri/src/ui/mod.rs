...
fn draw_status_bar(frame: &mut Frame<'_>, app: &App, area: Rect, theme: &Theme) {
    ...
    let hint = if app.popup().is_some() {
        ...
        match app.mode() {
            ...
            Mode::Config(_) => "↑/↓ select · Enter edit · ←/→ cycle · Ctrl+S save · Esc back".to_owned(),
            Mode::TaskList => "↑/↓ move · Enter open · [ ] move task · n new · , settings · ? keys · q quit".to_owned(),
            Mode::Task => match app.active_context().map(|c| c.focus) {
                Some(Focus::Terminal) if app.leader_pending() => {
                    format!("{leader} + q leave · z zoom · n new · x close · h/l switch · ? keys")
                }
                Some(Focus::Terminal) => format!("keys go to the shell · {leader} then q to leave · {leader} z zoom"),
                Some(Focus::Editor) => "Ctrl+S save · Ctrl+W close · Esc files · Tab next pane".to_owned(),
                Some(Focus::Shells) => "Enter focus · n new · x close · Tab next pane · Esc back".to_owned(),
                _ => "Enter open · a/A new · r rename · d delete · t shell · Tab next pane · Esc back".to_owned(),
            },
        }
    };
